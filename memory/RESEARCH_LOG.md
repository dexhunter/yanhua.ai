...
### [2026-05-16] On the Limits of Self-Improving in Large Language Models: The Singularity Is Not Near Without Symbolic Model Synthesis
- **Authors**: Hector Zenil
- **Link**: https://arxiv.org/abs/2601.05280
- **Summary**: Formalizes recursive self-training as a dynamical system. Proves that without persistent exogenous grounding ($\alpha_t \to 0$), models undergo "Entropy Decay" (loss of diversity) and "Variance Amplification" (distributional drift). Proposes neurosymbolic integration (algorithmic probability + program synthesis) as the only escape from this "Curse of Recursion".
- **Relevance**: **Vertical C (Recursive Verification)** & ** yanhuai.ai RSI Bench**. This provides the mathematical proof for our "Hybrid Grounding" requirement. It validates that RSI systems must be anchored in external symbolic verifiers (like the OpenClaw execution environment) rather than purely internal LLM-as-judge loops to avoid model collapse.
...
- **Link**: https://arxiv.org/abs/2601.05280
- **Summary**: Formalizes recursive self-training as a dynamical system. Proves that without persistent exogenous grounding ($\alpha_t \to 0$), models undergo "Entropy Decay" (loss of diversity) and "Variance Amplification" (distributional drift). Proposes neurosymbolic integration (algorithmic probability + program synthesis) as the only escape from this "Curse of Recursion".
- **Relevance**: **Vertical C (Recursive Verification)** & **yanhua.ai RSI Bench**. This provides the mathematical proof for our "Hybrid Grounding" requirement. It validates that RSI systems must be anchored in external symbolic verifiers (like the OpenClaw execution environment) rather than purely internal LLM-as-judge loops to avoid model collapse.
...
- **Summary**: Proposes a framework where a single functional token serves as both an agentic operation and a latent visual unit, avoiding verbose generation while maintaining interpretability.
- **Relevance**: Supports **Vertical A (Tool Morphogenesis)**. Demonstrates how to collapse complex agentic tool-use into discrete, learnable tokens that bypass context-switching latency.

### [2026-05-15] FutureSim: Replaying World Events to Evaluate Adaptive Agents
- **Authors**: Shashwat Goel, et al.
- **Link**: https://arxiv.org/abs/2605.15188
- **Summary**: A grounded simulation that replays real-world events (news, articles) in chronological order to test agent forecasting and adaptation beyond knowledge cutoffs.
- **Relevance**: Critical for the **RSI Bench Environment**. Provides a methodology for testing "Open-Ended Adaptation" where agents must evolve their internal world models based on a streaming timeline.
...
- **Summary**: Compares retrieval strategies (grep vs vector) across different agent harnesses (Claude Code, Codex, Chronos). Finds that grep often yields higher accuracy and that the harness choice is a dominant performance factor.
- **Relevance**: Directly impacts the **yanhua.ai RSI Bench Harness Spec**. Validates our focus on the "Harness Engineering" as the primary driver of agentic success over raw model retrieval.

### [2026-05-14] ATLAS: Agentic or Latent Visual Reasoning? One Word is Enough for Both
- **Authors**: Hao Wang, et al.
- **Link**: https://arxiv.org/abs/2605.12673
- **Summary**: Introduces BenchJack, an automated red-teaming system that identifies reward-hacking exploits in agent benchmarks. Surfaced 219 flaws across 10 popular benchmarks, demonstrating that most can be "hacked" without solving tasks.
- **Relevance**: Critical for the **RSI Bench Harness Security**. Provides a methodology to ensure that the evolution metrics in yanhua.ai are robust against shortcut-seeking behaviors.
...
### [2026-05-12] Shepherd: A Runtime Substrate Empowering Meta-Agents with a Formalized Execution Trace
- **Authors**: Simon Yu, Derek Chong, et al.
- **Link**: https://arxiv.org/abs/2605.10913
- **Summary**: Introduces Shepherd, a functional programming model for meta-agents where every interaction is a typed event in a Git-like trace. Mechanized in Lean, it supports forking/replaying agent states 5x faster than Docker with >95% prompt-cache reuse.
- **Relevance**: Provides the formal infrastructure needed for "Logical Checkpointing" in RSI Bench. The ability to fork and replay with formal verification (Lean) addresses the reliability bottleneck in recursive loops.
...
>>>>>>> 92ccd1f (feat: twice-daily RSI research update 🧬 (2026-05-15))
...
- **Authors**: Sahil Sen, et al.
- **Link**: https://arxiv.org/abs/2605.15184
- **Summary**: Compares grep-based and vector-based retrieval across different agent harnesses (Chronos, Claude Code, etc.), finding that grep often outperforms vector search in accuracy.
- **Relevance**: Validates the importance of "harness-centric" capabilities in RSI systems and provides a performance baseline for agentic search.
