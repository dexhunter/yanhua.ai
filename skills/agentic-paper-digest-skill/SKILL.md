...
compatibility: Requires Python 3, network access, and either git or curl/wget for bootstrap. LLM access via OPENAI_API_KEY or LLM_API_KEY (OpenAI-compatible).
...
- LLM access via `OPENAI_API_KEY` or an OpenAI-compatible provider via `LLM_API_BASE` + `LLM_API_KEY`.
...
- `OPENAI_API_KEY`: required for OpenAI models (llm reads this).
- `LLM_API_BASE`, `LLM_API_KEY`: use an OpenAI-compatible proxy/provider.
- `LLM_MODEL_RELEVANCE`, `LLM_MODEL_SUMMARY`: models for relevance and summarization (summary defaults to relevance model if unset).
- `LLM_TEMPERATURE_RELEVANCE`, `LLM_TEMPERATURE_SUMMARY`: lower for more deterministic output.
- `LLM_MAX_RETRIES`: retry count for LLM calls.
- `LLM_DROP_PARAMS=1`: drop unsupported params to avoid provider errors.
...
2. **ASK THE USER TO PROVIDE IT'S PREFERENCES ABOUT THE FOLLOWING (HELP THE USER)**:
   ...
   - **Model/provider** → set `OPENAI_API_KEY` *or* `LLM_API_KEY` (+ `LLM_API_BASE` if proxy), and set `LLM_MODEL_RELEVANCE`/`LLM_MODEL_SUMMARY`.
...
5. **Create or verify `.env`**:
   ...
   - Ensure at least one of `OPENAI_API_KEY` or `LLM_API_KEY` is set before running.
...
- Missing API key errors: export `OPENAI_API_KEY` or `LLM_API_KEY` in the shell before running.
